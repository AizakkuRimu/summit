...
(function () {
  ...
  // line-wraps have already been merged back together upstream by
  // mergeWordLineWraps, so by the time we get here every remaining
  // <p>/<div> in a Word paste is a real, deliberate paragraph break —
  // not a visual-line artifact — and shouldn't be flowed into its
  // neighbours the way a generic "line per block" source would be.
  // Instead each one becomes its own paragraph, with a blank spacer
  // paragraph inserted between consecutive ones for visual breathing
  // room (an explicit blank line already in the source still becomes
  // just one spacer, same as the non-Word path).
  function flattenPasteLineBreaks(root, preserveParagraphs) {
    ...
    function toStandaloneParagraph(n) {
      const p = document.createElement('p');
      appendNodeAsProse(p, n);
      trimMergedEdge(p, true);
      trimMergedEdge(p, false);
      return p;
    }
    ...
    function mergeRun(container, run) {
      ...
      groups.forEach((g) => {
        ...
        if (preserveParagraphs) {
          g.forEach((n, idx) => {
            replacements.push(toStandaloneParagraph(n));
            if (idx < g.length - 1) replacements.push(document.createElement('p'));
          });
          return;
        }
