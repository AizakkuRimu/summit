...
(function () {
  ...
  function rankFindResults(results, q) {
    ...
    const scored = results.map((r) => {
      ...
      const textHits = Math.min(5, r.sub.template.toLowerCase().split(lowerQ).length - 1);
  ...
  function renderViewMoreBatch() {
    ...
    for (let i = start; i < end; i++) {
      viewMoreResultsEl.appendChild(renderResultCard(findFullList[i].sub, findFullQuery, rerenderViewMoreLoaded));
    }
